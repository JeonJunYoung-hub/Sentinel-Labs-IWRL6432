...
공기질 노드([Sentinel-Labs-nRF5340-Wearable](https://github.com/JeonJunYoung-hub/Sentinel-Labs-nRF5340-Wearable))와 같은 저장/전송 구조(SNTL 프레임,
NVS 24시간 링, BLE 덤프)를 쓰되 레코드 내용만 재실 데이터로 바꾼 것이다. 프레임
**version 바이트가 2**라서 앱이 둘을 구분한다.

레이더 브링업 기록과 Python 도구는 같은 저장소 안에 있다: [../tools/README.md](../tools/README.md).
...
합성 프레임은 `tools/tlv.py` 의 자체 검증과 같은 값을 쓴다 — 양쪽 디코더가
같은 레이아웃을 본다는 뜻이다.
...
- [../tools/README.md](../tools/README.md) — 레이더 브링업 기록과 Python 도구
  (`radar.py` / `tlv.py` / `monitor.py`). **먼저 읽을 것.**
- [Sentinel-Labs-nRF5340-Wearable](https://github.com/JeonJunYoung-hub/Sentinel-Labs-nRF5340-Wearable) —
